...
#username = open( "username.yaml", "w")
#dsw1cfg = open( "newvlan_dsw1" + "_.cfg", "w")

#Load data from YAML into Python dictionary
config_data = yaml.load(open('./username.yaml'))

#Load Jinja2 template
env = Environment(loader = FileSystemLoader('./'), trim_blocks=True, lstrip_blocks=True)
username = env.get_template('username.j2')

#Render the template with data and print the output
print(username.render(config_data))
#dsw1cfg = open( "newvlan_dsw1" + "_.cfg", "w")
ok = (username.render(config_data))
...
for index in range(1, sheet.nrows):
    ...
    qashow = net_connect.send_config_set(ok)
    print(qashow)
