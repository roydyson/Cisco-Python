...
#loopback = open( "loopback.yaml", "w")
#dsw1cfg = open( "newvlan_dsw1" + "_.cfg", "w")

#Load data from YAML into Python dictionary
config_data = yaml.load(open('./loopback.yaml'))

#Load Jinja2 template
env = Environment(loader = FileSystemLoader('./'), trim_blocks=True, lstrip_blocks=True)
loopback = env.get_template('loopback.j2')

#Render the template with data and print the output
print(loopback.render(config_data))
#dsw1cfg = open( "newvlan_dsw1" + "_.cfg", "w")
cmd  = (loopback.render(config_data))
